...
fn main() {
    ...
    // let target_dir = std::path::Path::new(&out_dir).join("../../../deps");
    ...
    //         "build",
    //         "--manifest-path",
    //         manifest_dir_path.join("Cargo.toml").to_str().unwrap(),
    //         "--package",
    //         "ninja-rs",
    //         "--lib",
    //         "--target-dir",
    //         sub_target_dir.to_str().unwrap(), // CRITICAL: Separate target dir
    //         "--target",
    //         &target_triple,
    //         "--release",
    //     ])
    ...
    if is_windows && build.get_compiler().is_like_msvc() {
        // defines
        ...
        let disabled_warnings = [
            "4530", "4100", "4706", "4244", "4512", "4800", "4702", "4127", "4355", "4091", "4820",
            "4514",
        ];
    ...
    for file in src_files.iter() {
        ...
        // dbg!(o_filename);
        // panic!();
        let entry = entries.iter().find(|direntry| {
            direntry
                .file_name()
                .into_string()
                .unwrap()
