...
pub unsafe fn multiple_hex_to_u64_sse(input: &[[u8;16]],output: &mut [u64]){
    ...
    let simd_loadfn = {if addr %16 == 0 {_mm_load_si128} else {_mm_loadu_si128}};
...
#[allow(non_snake_case)]
/// the caller MUST ensure this fn is called with [u8;16] or will cause UB
pub fn ninja_rs__build_log__parse__fast_hex_multiple_to_u64(input: &[[u8;16]],output: &mut [u64]) {
...
pub fn test_fast_hex_to_u64() {
    fn run_test(input: &str) {
        ...
        let r_output = ninja_rs__build_log__parse__fast_hex_single_to_u64(input.as_bytes().try_into().unwrap());
...
pub fn fuzz_hex_parser() {
    ...
    for i in 0..ITERS {
        ...
        let expected = unsafe { strtoull(c_string.as_ptr(), std::ptr::null_mut(), 16) };
    ...
    std::hint::black_box(ninja_rs__build_log__parse__fast_hex_multiple_to_u64(inputs.as_slice(), &mut ouptuts));
    let elapsed = start.elapsed().as_nanos();
    for i in 0..ITERS {
        let expected = unsafe {c_strtoulls.get_unchecked(i)};
        let output = unsafe {ouptuts.get_unchecked(i)};
        if expected !=output {
            panic!("Test failed {expected} !={output}");
        }
    }
    // const BATCH:usize = (ITERS.ilog2() as usize) * 32;
    println!("Whole parse took : {:.2} ns average for {ITERS} iters", elapsed as f64 / ITERS as f64);
}
